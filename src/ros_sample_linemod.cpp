#include <opencv_linemod_test/ros_sample_linemod.hpp>
...
void imageCallback(const sensor_msgs::Image::ConstPtr &ros_rgb_image, const sensor_msgs::Image::ConstPtr &ros_depth_image){
  
  // convert ros image to cv image
  cv::Mat color, depth;
  try{
    color = cv_bridge::toCvCopy(ros_rgb_image)->image;
    depth = cv_bridge::toCvCopy(ros_depth_image)->image;
//     depth.convertTo(depth, CV_32FC1, 0.001);
  ...
  // pushing sources (color and depth)
...
//   std::string type_depth =  type2str( depth.type() );
//   ROS_INFO("Matrix depth: %s %dx%d \n", type_depth.c_str(), depth.cols, depth.rows );
//   std::string type_color =  type2str( color.type() );
//   ROS_INFO("Matrix depth: %s %dx%d \n", type_color.c_str(), color.cols, color.rows );
  ...
  if (!learn_online)
  {
    ...
    if (event == CV_EVENT_RBUTTONDOWN)
    {
      ...
      cv::imshow("mask", mask);
//       std::string ty =  type2str( mask.type() );
...
{
  ros::init(argc, argv, "ros_sample_linemod");
  ros::NodeHandle nh;
  
  // ApproximateTime synchronized callback of point cloud and depth image
  message_filters::Subscriber<sensor_msgs::Image> first_sub(nh, "/camera/rgb/image_color", 1);
  message_filters::Subscriber<sensor_msgs::Image> second_sub(nh, "/camera/depth_registered/image_raw", 1);
  ...
  if (argc == 1)
  {
    ROS_INFO("No template passed as arguments");
    filename = "linemod_templates.yml";
    detector = cv::linemod::getDefaultLINEMOD();
  }
  else
  {
    ROS_INFO("Reading templates");
  ...
  focal_length = (640 * 0.5)/tan(58*0.5*3.14159265359/180); // WIDTH * 0.5 / tan(FOV * 0.5* PI/180)
